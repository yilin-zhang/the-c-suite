...
Skills and plugins for Claude Code, OpenCode, and Codex.
...
registered via `claude plugin marketplace add`. OpenCode plugins are
symlinked into `~/.config/opencode/plugin/`.

Skills update instantly on `git pull` (symlinks). Claude Code plugins
need to be upgraded inside Claude Code after pulling. OpenCode plugins
update instantly (symlinks).
...
| skillify | codex, opencode | Capture a repeatable process as a reusable skill |
...
| emacsclient | all | Always use emacsclient for Emacs operations |
| epub-cleanup | all | Clean and normalize EPUB/KEPUB files |
| index-codebase | all | Generate structured feature specs for a codebase |
...
├── oc-plugins/           # OpenCode plugins (read each README for setup)
